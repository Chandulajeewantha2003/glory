...
class WeightActivity : AppCompatActivity() {
    ...
    private var selectedWeight = 76 // kg default
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        selectedWeight = savedInstanceState?.getInt("weight_kg") ?: selectedWeight

        tvPrev = findViewById(R.id.tvPrev)
        tvSelected = findViewById(R.id.tvSelected)
        tvNext = findViewById(R.id.tvNext)
        np = findViewById(R.id.npWeight)
        ...
        findViewById<MaterialButton>(R.id.btnContinue).setOnClickListener {
            // TODO: navigate; use selectedWeight (kg)
            // startActivity(Intent(this, NextActivity::class.java).putExtra("weight_kg", selectedWeight))
            finish()
        }
        val skipButton: Button = findViewById(R.id.btnContinue)
        skipButton.setOnClickListener {
            val intent = Intent(this, TargetWeightActivity::class.java)
    ...
    private fun setupPicker() {
        ...
        val startIdx = (selectedWeight - 30).coerceIn(0, values.lastIndex)
        np.value = startIdx
        updateOverlay(startIdx)

        np.setOnValueChangedListener { _, _, newIndex ->
            selectedWeight = 30 + newIndex
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        outState.putInt("weight_kg", selectedWeight)
